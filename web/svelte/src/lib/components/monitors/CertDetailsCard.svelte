<script lang="ts">
	import { Lock, AlertTriangle, CheckCircle2, XCircle } from 'lucide-svelte';
	...
	onMount(async () => {
		try {
			const res = await monitorsApi.getCertDetails(monitorId);
			cert = res.data;
		} catch {
			failed = true;
		} finally {
...
{#if loading}
	<section>
		<div class="animate-pulse space-y-3">
			<div class="h-4 w-1/3 rounded bg-muted/50"></div>
			<div class="h-3 w-2/3 rounded bg-muted/30"></div>
			...
			<div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
				...
				<div class="space-y-1">
					...
					<div class="flex items-center space-x-1.5">
						...
						{/if}
						<p class="text-lg font-semibold font-mono {expiryColor(cert.expiry_days)}">
				...
				<div class="space-y-1">
					...
					<div class="flex items-center space-x-1.5">
						{#if cert.chain_valid}
							...
							<span class="text-sm text-emerald-400 font-medium">Valid</span>
						{:else}
							<XCircle class="w-3.5 h-3.5 text-red-400" />
							<span class="text-sm text-red-400 font-medium">Invalid</span>
						{/if}
					</div>
				</div>
